...
export function VerifyEmailPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0A0A0A] text-[#F3F1EA] flex flex-col justify-between p-4 sm:p-6 lg:p-8 overflow-x-hidden">
      <header className="flex flex-wrap items-center gap-4 pb-6 border-b border-[#242522]">
        <Link to="/" aria-label="SignalFold Home"><BrandLogo size="md" /></Link>
        <span className="min-w-0 font-mono text-[10px] tracking-widest text-[#A8AAA3] uppercase">AUTHENTICATION / VERIFY EMAIL</span>
      </header>
      <main className="w-full flex-1 flex items-center justify-center py-12 sm:py-16">
        <div className="w-full max-w-lg min-w-0 mx-auto bg-[#141513]/40 border border-[#242522] p-6 sm:p-8 rounded-[2px] space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => void verify()} disabled={pending || !safeEmail || !otp || isMockMode} className="min-w-0 w-full px-3 sm:px-4 py-3 text-xs font-mono font-bold tracking-widest bg-[#F3F1EA] text-[#0A0A0A] rounded-[2px] disabled:opacity-50">VERIFY</button>
            <button type="button" onClick={() => void resend()} disabled={pending || !safeEmail || isMockMode} className="min-w-0 w-full px-3 sm:px-4 py-3 text-xs font-mono font-bold tracking-widest border border-[#242522] text-[#A8AAA3] rounded-[2px] disabled:opacity-50">RESEND</button>
          </div>
          <Link to="/signup" className="inline-flex whitespace-nowrap text-xs text-[#A8AAA3] hover:text-[#D6FF3F]">BACK TO SIGNUP</Link>
        </div>
      </main>
